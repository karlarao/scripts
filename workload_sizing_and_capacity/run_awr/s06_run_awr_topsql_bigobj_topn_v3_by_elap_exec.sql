-- run_awr_topsql_bigobj_topn_v3_by_elap_exec.sql
...
spool awr_topsql_bigobj_by_elap_exec-&_instname-&_conname-&_conid..csv
...
select trim('&_dbname') db , a.*, st.sqldetail sqldetail from
                          (
                            SELECT s.parsing_schema_name pschema,
                                   ...
                                   DENSE_RANK() OVER (PARTITION BY spacesql.object_name, stt.sql_text ORDER BY decode((sum(s.executions_delta)), 0, to_number(null), ((sum(s.elapsed_time_delta)) / (sum(s.executions_delta)) / 1000000)) DESC) time_rank
