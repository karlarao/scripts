...
accept goodsql_id -
       prompt 'Enter value for goodsql_id: ' -
       default 'X0X0X0X0'
accept goodchild_no -
       prompt 'Enter value for goodchild_no (0): ' -
       default '0'
accept badsql_id -
       prompt 'Enter value for badsql_id: ' -
       default 'X0X0X0X0'
accept badchild_no -
       prompt 'Enter value for badchild_no (0): ' -
       default '0'
accept profile_name -
...
select
sql_fulltext,
decode('&&profile_name','X0X0X0X0','PROF_&&badsql_id'||'_'||plan_hash_value,'&&profile_name')
into
...
when NO_DATA_FOUND then
  dbms_output.put_line(' ');
  dbms_output.put_line('ERROR: badsql_id: '||'&&badsql_id'||' Child: '||'&&badchild_no'||' not found in v$sql.');
  dbms_output.put_line(' ');
