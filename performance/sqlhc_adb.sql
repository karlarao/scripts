...
REM
Rem $Header: pdbcs/no_ship_src/service/scripts/ops/adb_sql/diagsql/sqlhc.sql /main/4 2022/04/27 13:03:21 kdrupare Exp $
REM
...
Rem    scharala    05/23/22 - Collect 10053 when the user is SYS
Rem    kdrupare    04/25/22 - BUG 34069157: COLLECT LAST_DDL for dependent
Rem                           objects
...
PRO <html>
PRO <!-- $Header: pdbcs/no_ship_src/service/scripts/ops/adb_sql/diagsql/sqlhc.sql /main/4 2022/04/27 13:03:21 kdrupare Exp $ -->
...
SELECT CHR(10)||'<tr>'||CHR(10)||
       ...
       '<td class="r">'||v.last_ddl_time||'</td>'||CHR(10)||
       '</tr>'
  FROM (
        SELECT /*+ NO_MERGE */ d.to_owner, d.to_name, d.to_type,
               decode(d.to_type, 
                       0,'CURSOR',
                       1,'INDEX',
                       2,'TABLE', 
                     ...
                     ,TO_CHAR(TO_DATE(o.LAST_DDL_TIME, 'DD-MON-YY HH24:MI:SS'), 
                       'DD-MON-YY HH24:MI:SS') LAST_DDL_TIME
          FROM gv$object_dependency d
              ,dba_objects o
         WHERE 1=1
           -- and to_owner <> 'SYS'
...
-- generates script that creates sql profile in target system:
	BEGIN
		...
		DBMS_OUTPUT.PUT_LINE('REM');
		  ...
		  DBMS_OUTPUT.PUT_LINE('REM');
		  DBMS_OUTPUT.PUT_LINE('REM $Header: pdbcs/no_ship_src/service/scripts/ops/adb_sql/diagsql/sqlhc.sql /main/4 2022/04/27 13:03:21 kdrupare Exp $');
		  DBMS_OUTPUT.PUT_LINE('REM');
...
spool off
spool sqlhc_10053.log
--
-- Identify the currently running schema
-- If the currently running schema is not SYS then write an warning message and continue
--

column sv_trace_dir new_value sv_trace_dir format a500
column sv_cp_cmd new_value sv_cp_cmd format a500
column sv_current_user new_value sv_current_user format a128

select value sv_trace_dir from v$diag_info where name='Diag Trace';
select sys_context('USERENV', 'CURRENT_USER') sv_current_user FROM dual;
variable bv_current_user varchar2(128);
exec :bv_current_user := '^^sv_current_user.';

set serveroutput on
declare
  bv_current_user varchar2(128);
begin
  if ( :bv_current_user = 'SYS') then
    dbms_output.put_line('Collecting trace...');
...
select case ( select '^^sv_current_user.' from dual )
  when 'SYS' then 'cp -v '||'^^sv_trace_dir.'||'/*TRACE_10053*.trc .'
  else 'touch ./TRACE_10053_NOT_COLLECTED_FOR_NON_SYS_USER.trc'
  end sv_cp_cmd from dual;

host ^^sv_cp_cmd.

undefine sv_trace_dir sv_cp_cmd sv_current_user
host zip -m optimizer_trace.zip ./*TRACE_10053*.*
...
host zip -m ^^files_prefix._9_log.zip sqlhc_10053.log
...
host zip -m ^^files_prefix._18_10053.zip ./optimizer_trace.zip 
host zip -m ^^files_prefix..zip ^^files_prefix._18_10053.zip
